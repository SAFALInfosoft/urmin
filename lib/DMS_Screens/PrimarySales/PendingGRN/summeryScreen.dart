...
class OrderSummaryScreen extends StatefulWidget {
  ...
  double orderTotal;
  var item;
  double temp_roundoff;
  double totalofScemeDiscount;
  ...
  OrderSummaryScreen(
    ...
    this.item,
    this.temp_roundoff,
    this.totalofScemeDiscount,
    this.LocalFieldString, {
    Key? key,
...
class _MyHomePageState extends State<OrderSummaryScreen> {
  ...
  openHiveBox() async {
    ...
    bookmark.forEach((key, value) {
      ...
      if (key == 'docs') {
        debugPrint(
            "DATA ${value[0]['gst'][0]['state_code'] + companyStateCode}");
        setState(() {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      int desiredIndex =
          0; // Replace 0 with the index you want to calculate total box quantity for
      List<PurchaseOrder> purchaseOrders = List<PurchaseOrder>.from(items);
      double totalBoxQuantity =
          calculateTotalBoxQuantity(purchaseOrders, desiredIndex);
...
//     }
    ...
    roundOff =
        (grandTotal - widget.total.toDouble() - widget.gstCharge.toDouble());
          ...
          children: [
            ...
            Flexible(
              child: InkWell(
                ...
                onTap: () async {
                  if(compneyNameController.text!= null && compneyNameController.text != "") {
                    WidgetsBinding.instance
                        .addPostFrameCallback((timeStamp) async {
                      log(widget.LocalFieldString);

                      Map<String, dynamic> dataList =
                      jsonDecode(widget.LocalFieldString);
                      var data = dataList;
                      ...
                      if (draftList == null || draftList.isEmpty) {
                        ...
                        var matchingDraft = draftList?.firstWhere(
                              (draftJsonString) {
                            var draftData = jsonDecode(draftJsonString);
                            print("data['urnNo']: ${data['urnNo']}");
                            print("draftData['urnNo']: ${draftData['urnNo']}");
                            return draftData['urnNo'] == data['urnNo'];
                          },
                          orElse: () => "",
                        ) as String?;
                        //
                        // // Remove the matching PurchaseOrder from draftList
                        if (matchingDraft != null) {
                          existingData = prefs.getStringList("POLISTJSON") ?? [
                          ];
                          draftList?.remove(matchingDraft);

                          // Add the matching PurchaseOrder to another list (e.g., existingData)

                          existingData?.add(matchingDraft);


                          // Save the modified lists back to SharedPreferences
                          prefs.setStringList(
                              "PODRAFTJSON", draftList.cast<String>());
                          prefs.setStringList(
                              "POLISTJSON", existingData.cast<String>());
                        }
                        /*else if (draftList== null )
                     {
                      ...
                      prefs.setStringList("POLISTJSON", existingData!);
                        ...
                        print(existingData?.length.toString() ?? "0");
                        Navigator.push(
                            context,
                            MaterialPageRoute(
                                builder: (context) => po_Tab_View()));
          ...
          child: Column(
            ...
            children: <Widget>[
                ...
                children: <Widget>[
                  Flexible(
                    child: Column(
                      ...
                      children: <Widget>[
                        Text('Total Of WSP'),
                        widget.totalofScemeDiscount != 0.00
                            ? SizedBox(
                                height: 5.0,
                              )
                            : SizedBox.shrink(),
                        ...
                        (companyStateCode != Distributor_state_code) ||
                                (Distributor_state_code != "24" &&
                                    companyStateCode != "24")
                            ? Text('IGST')
                            : SizedBox.shrink(),
                        (companyStateCode != Distributor_state_code) ||
                  ...
                  Column(
                    ...
                    children: <Widget>[
                      Text(
                          '  \u{20B9}${widget.total.toDouble().toStringAsFixed(2)}',
                      ...
                      widget.totalofScemeDiscount != 0.00
                          ? Text(
                              '  \u{20B9}${widget.totalofScemeDiscount.toDouble().toStringAsFixed(2)}',
                              style: TextStyle(overflow: TextOverflow.ellipsis))
                          : SizedBox.shrink(),
                      SizedBox(
                        height: 5.0,
                      ...
                      (companyStateCode != Distributor_state_code) ||
                              (Distributor_state_code != "24" &&
                                  companyStateCode != "24")
                          ? Text(
                              '  \u{20B9}${widget.IGST.toDouble().toStringAsFixed(2)}',
                              style: TextStyle(overflow: TextOverflow.ellipsis))
                          : SizedBox.shrink(),
                      (companyStateCode != Distributor_state_code) ||
                              (Distributor_state_code != "24" &&
                      ...
                      (companyStateCode == Distributor_state_code) ||
                              (Distributor_state_code == "24" &&
                                  companyStateCode == "24")
                          ? Text(
                              '  \u{20B9}${widget.CGST.toDouble().toStringAsFixed(2)}',
                              style: TextStyle(overflow: TextOverflow.ellipsis))
                          : SizedBox.shrink(),
                      (companyStateCode == Distributor_state_code) ||
                              (Distributor_state_code == "24" &&
                      ...
                      (companyStateCode == Distributor_state_code) ||
                              (Distributor_state_code == "24" &&
                                  companyStateCode == "24")
                          ? Text(
                              '  \u{20B9}${widget.SGST.toDouble().toStringAsFixed(2)}',
                              style: TextStyle(overflow: TextOverflow.ellipsis))
                          : SizedBox.shrink(),
                      (companyStateCode == Distributor_state_code) ||
                              (Distributor_state_code == "24" &&
                          ...
                          : SizedBox.shrink(),
                      Text(
                          '  \u{20B9}${widget.gstCharge.toDouble().toStringAsFixed(2)}'),
                      SizedBox(height: 5.0),
                      Text(
                          '  \u{20B9}${widget.TCS.toDouble().toStringAsFixed(2)}'),
                      SizedBox(
                        height: 5.0,
                      ),
                      Text(
                          '  \u{20B9}${widget.temp_roundoff.toDouble().toStringAsFixed(2)}'),
                      SizedBox(
                        height: 5.0,
                      ),
                      Text('  \u{20B9}${widget.orderTotal.roundToDouble()}',
                          style: TextStyle(overflow: TextOverflow.ellipsis)),
                    ],
                  ),
                ],
                        ...
                        selectionItems.length > 1
                            ? PopupMenuButton(
                                ...
                                onSelected: (value) {
                                  setState(() {
                                    var selectedItem =
                                        selectionItems.firstWhere((item) =>
              ...
              TextFormField(
                ...
                // Set keyboardType to TextInputType.multiline
                style: TextStyle(
                  fontWeight: FontWeight.w500,
                  fontSize: 14,
                  color: Colors.black,
